            ...
            <div className="relative w-full h-[100dvh] overflow-hidden">
                ...
                <div className={`absolute top-4 left-1/2 -translate-x-1/2 z-20 transition-all duration-700 ease-out transform pointer-events-none ${isFilterMounted && (!isMobile || !selectedPropertyId) ? 'translate-y-0 opacity-100' : '-translate-y-12 opacity-0'}`}>
                    <div className="flex flex-col items-center gap-2">
                        <div className={`flex items-center bg-[#F9F7F2]/95 backdrop-blur-[24px] border border-[#1B1B1B]/10 rounded-full p-1.5 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.15)] pointer-events-auto transition-all duration-500 ease-in-out ${isFilterMobileExpanded ? 'max-w-[92vw]' : 'max-w-[120px] md:max-w-none'}`}>
                            <button
                            ...
                            <div className={`${isFilterMobileExpanded ? 'flex' : 'hidden md:flex'} items-center gap-1.5 px-1 overflow-x-auto no-scrollbar`}>
                                ...
                                <button onClick={clearStatusFilters} className="flex-shrink-0 px-3 py-2.5 rounded-full text-[9px] font-bold uppercase tracking-widest text-[#E08E5F] hover:bg-[#E08E5F]/10 transition-colors">Clear</button>
                            </div>
                        </div>
                    </div>
                </div>
