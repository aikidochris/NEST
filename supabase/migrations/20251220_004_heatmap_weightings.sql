-- VERSION 3.0.0 (HEATMAP-READY MVT)
...
with
  ...
  pts as (
    select
      ...
      CASE 
        ...
        ELSE 'unclaimed' 
      END::text as status,
      -- ANALYTICS WEIGHT: Higher value = brighter heatmap aura
      CASE 
        WHEN pv.is_for_sale THEN 1.0
        WHEN pv.is_open_to_talking THEN 0.8
        WHEN pv.is_claimed THEN 0.4
        ELSE 0.1
      END::float as discovery_weight,
      (COALESCE(pv.is_for_sale, false) OR COALESCE(pv.is_open_to_talking, false))::boolean as has_active_intent,
  ...
  clipped as (
    select p.property_id, p.status, p.discovery_weight, p.has_active_intent,
